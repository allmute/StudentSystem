...
public class GradeidQuery extends JFrame {
	{
		try {
		...
		this.button_a.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {  //判断学号信息是否存在
					...
					String sql = "select count(*) AS num FROM sc WHERE id=?";
					...
					if(num == 0){
						JOptionPane.showMessageDialog(null, "此学号的学生信息不存在!", "错误",
								JOptionPane.ERROR_MESSAGE);
						return;
					}else{

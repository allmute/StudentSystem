...
public class GradeclassQuery extends JFrame {
	{
		try {
	...
	public GradeclassQuery() {
		...
		this.label = new JLabel("请输入你的班级编号:");
		...
		this.button_a.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try { // 判断班级编号信息是否存在
					...
					String sql = "select count(*) AS num FROM sc WHERE classid=?";
					...
					if (num == 0) {
						JOptionPane.showMessageDialog(null, "此班级编号不存在!",
...
//		this.setDefaultCloseOperation(EXIT_ON_CLOSE);
	...
	public void btnShow_ActionPerformed(ActionEvent ae) {
		try {
			...
			String sql = "SELECT * FROM sc WHERE classid ='" + classid + "'";
